...
class pyUR(BulletClient):
    ...
    def __init__(self, config="default.yaml"):
        ...
        if self.config.skin.use:
            ...
            self.link_to_skin_string = {"airskin_0": ["airskin_0"], "airskin_1": ["airskin_1"],
                                        "airskin_2": ["airskin_2"], "airskin_3": ["airskin_3"],
                                        "airskin_4": ["airskin_4"], "airskin_5": ["airskin_5"],
                                        "airskin_6": ["airskin_6"], "airskin_7": ["airskin_7"],
                                        "airskin_8": ["airskin_8"], "airskin_9": ["airskin_9"],
                                        "onrobot_rg6_base_link": ["airskin_10"],
                                        "left_outer_knuckle": ["airskin_10"], "right_outer_knuckle": ["airskin_10"],
                                        "left_inner_knuckle": ["airskin_10"], "right_inner_knuckle": ["airskin_10"],
                                        "wrist_3_link": ["airskin_10"],
                                        #"left_inner_finger": ["airskin_11"], "right_inner_finger": ["airskin_11"]
                                        }
            self.visualizer_skin_path = {}
            for f_path in self.activated_skin:
                skin_name = f_path.split("_vhacd")[0].split("/")[-1]
                skin_part_to_show = []
                for robot_link, skin_link in self.link_to_skin_string.items():
                    if skin_name in skin_link:
                        for link in self.links:
                            if link.name == robot_link:
                                f_path_ = link.urdf_link.collision.geometry.mesh.filename
                                break
                        skin_part_to_show.append(f_path_)

                self.visualizer_skin_path[f_path] = skin_part_to_show
        ...
        for joint in range(self.getNumJoints(self.robot)):
            info = self.getJointInfo(self.robot, joint)
            if str(info[self.jointInfo["NAME"]], "utf-8") == "wrist_ft_sensor_joint":
    ...
    def init_robot(self):
        ...
        for obj_id, obj_name, fixed, _, _ in self.other_objects:
            if fixed:
                for obj_link_id in np.array(range(len(self.urdfs[obj_name].links)))-1:
    ...
    def init_urdfs(self):
        ...
        if hasattr(self.config, "urdfs"):
            for urdf_id, urdf, pos, color, fixed in zip(range(len(self.config.urdfs.paths)), self.config.urdfs.paths, self.config.urdfs.positions, self.config.urdfs.color, self.config.urdfs.fixed):
                ...
                self.other_objects.append((self.loadURDF(urdf, pos, useFixedBase=fixed), obj_name, fixed, color, urdf.split("other_meshes/")[1].replace(".urdf", ".obj")))

                if not fixed:
                    self.free_objects.append(self.other_objects[-1][0])
                #print(obj_name, self.getDynamicsInfo(self.other_objects[-1][0], -1))
    ...
    def update_simulation(self, sleep_duration=0.01):
        ...
        if sleep_duration is None or time.time()-self.last_step > sleep_duration:
            ...
            #print(np.abs(info[self.jointStates["TORQUE"]]))
            #if np.abs(info[self.jointStates["TORQUE"]]) > 3:#self.gripper.max_force:
    ...
    def compute_skin(self):
        ...
        for other_obj, _, _, _, _ in self.other_objects:
            close_points = self.getClosestPoints(self.robot, other_obj, 0.005)
            ...
            for pair in close_points:
                link = pair[3]
                for link__ in self.links:
                    if link == link__.robot_link_id:
                        break
                if link in self.link_to_skin:
                    if link in links_in_contact:
                        links_in_contact[link].append([pair[9], pair[4], pair[8]])
                    else:
                        links_in_contact[link] = [[pair[9], pair[4], pair[8]]] #force, id, dist

            # if contact detected, get the lowest distance and update the skin activation
            if len(links_in_contact) > 0:
                for link_in_contact in links_in_contact:
                    for f_path in self.activated_skin:
                        f_path_comp = f_path.split("_vhacd")[0].split("/")[-1]

                        if self.link_to_skin_string[self.link_to_skin[link_in_contact][0]][0].split("__")[0] == f_path_comp:
                            dist = np.min(np.array(links_in_contact[link_in_contact])[:, 2])  # the minimal distance is the one we want
                            self.activated_skin[f_path] = dist*10000  # some magic to get a nice number
                            break
            ...
            for link in obj.links:
                if hasattr(link, "collision"):
                    if hasattr(link.collision.geometry, "mesh"):
                        ...
                        if not robot and self.config.vhacd.force_vhacd or not os.path.exists(vhacd_path):
                            self.vhacd(col_path, vhacd_path, "", resolution=1000000, maxNumVerticesPerCH=128, gamma=0.0005, concavity=0)
    ...
    def create_urdf(self, object_path, fixed, color, suffix=""):
        ...
        urdf = urdf.replace("OBJECTNAME", model_name).replace("LATERALFRICTION", "10") \
            .replace("ROLLINGFRICTION", "10").replace("SPINNINGFRICTION", "10").replace("MASS", "1").replace("FILENAMECOLLISION", object_path) \
