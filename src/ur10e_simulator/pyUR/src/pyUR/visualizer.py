...
class Visualizer:
    ...
    def __init__(self, client=None):
        ...
        # Prepare the menu
        self.save_menu = gui.Menu()
        self.save_menu.add_item("RGB image", 0)
        self.save_menu.add_item("depth image", 1)
        menu_counter = 2
        self.menu.add_menu("Save", self.save_menu)
        ...
        self.show_menu.add_item("Coordination", 2)
        c = self.MenuCallback(2, self)
        self.window.set_on_menu_item_activated(2, c)
        self.menu.add_menu("Show", self.show_menu)
        self.menu.set_checked(2, False)
    ...
    def show_first(self, urdf_name="robot"):
        ...
        for mesh_id in range(len(self.positions) // 3):
            ...
            if self.client.config.skin.use:
                f_path_split = f_path.split("_vhacd")[0].split("/")[-1]
                for skin_link_name, skin_links in self.client.visualizer_skin_path.items():
                    for skin_link in skin_links:
                        if f_path_split == skin_link.split("_vhacd")[0].split("/")[-1]:
                            ...
                            self.meshes[f_path].paint_uniform_color([0, 0, 1])
                            self.vis.add_geometry(f_path_on, geometry=self.meshes[f_path_on], material=self.mat)
                            self.vis.show_geometry(f_path_on, False)
                            if skin_link_name not in self.skin_link_to_all_link:
                                self.skin_link_to_all_link[skin_link_name] = []
                            self.skin_link_to_all_link[skin_link_name].append(f_path)
                            if f_path not in self.all_skin:
                                self.all_skin.append(f_path)
                            break
    ...
    def show_mesh(self):
        ...
        for mesh_id in range(len(self.positions) // 3):
            ...
            if self.client.config.skin.use and f_path in self.all_skin:
                self.vis.set_geometry_transform(f_path.replace(".obj", "_on.obj"), R @ R_urdf)

        if self.client.config.skin.use:
            for f_path, on in self.client.activated_skin.items():
                for f_path_link in self.skin_link_to_all_link[f_path]:
                    self.vis.show_geometry(f_path_link, not on)
                    self.vis.show_geometry(f_path_link.replace(".obj", "_on.obj"), on)
